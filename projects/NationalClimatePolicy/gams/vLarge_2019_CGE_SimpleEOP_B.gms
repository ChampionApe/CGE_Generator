...
vA_F.fx[s]$(HH_sm[s]) = vA_F.l[s]$(HH_sm[s]);
uIdxFund.fx[s]$(HH_sm[s]) = uIdxFund.l[s]$(HH_sm[s]);
...
vA.lo[t,s]$(((HH_sm[s] and tx0[t]) or (HH_sm[s] and t0[t]))) = -inf;
vA.up[t,s]$(((HH_sm[s] and tx0[t]) or (HH_sm[s] and t0[t]))) = inf;
...
# solve:
 solve M_vLarge_2019_CGE_SimpleEOP_B using CNS;
