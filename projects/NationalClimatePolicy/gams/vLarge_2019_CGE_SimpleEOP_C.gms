...
E_P_pWedge_pwOut[t,s,n]$(p_output[s,n] and txe[t]).. 		p[t,n] 			 =E=  pS[t,s,n]+(tauEffCO2[t,s,n]*uCO2[t,s,n])$(dqCO2[s,n])+tauS[t,s,n]+jTerm[s];
...
E_I_pWedge_pwOut[t,s,n]$(i_output[s,n] and txe[t]).. 		p[t,n] 			 =E=  pS[t,s,n]+tauS[t,s,n]+jTerm[s];
...
E_HH_vU_qC[t,s]$(hh_sm[s] and txe[t]).. 	qC[t,s]		 =E=  sum([n,nn]$(HH_L2C[s,n,nn]), qD[t,s,nn]-frisch[s]*(qS[t,s,n]/Lscale[s])**((1+frisch[s])/frisch[s])/(1+frisch[s]));
...
# ----------------------------------------------------------------------------------------------------
EQUATION E_HH_Euler_vA[t,s];
E_HH_Euler_vA[t,s]$(hh_sm[s] and txe[t]).. 		vA[t+1,s]	 =E=  (vA[t,s]*Rrate[t]+sp[t,s]+jTerm[s])/(1+g_LR);
EQUATION E_HH_Euler_vAT[t,s];
E_HH_Euler_vAT[t,s]$(hh_sm[s] and te[t]).. 		vA[t,s]		 =E=  (1+h_tvc[s])*vA[t-1,s];
...
# ----------------------------------------------------------------------------------------------------
Model B_HH_Euler /
E_HH_Euler_vA, E_HH_Euler_vAT, E_HH_Euler_euler
/;



# -------------------------------------------B_HH_vA0_Calib-------------------------------------------
#  Initialize B_HH_vA0_Calib equation block
# ----------------------------------------------------------------------------------------------------
EQUATION E_HH_vA0_Calib_vAt0[t,s];
E_HH_vA0_Calib_vAt0[t,s]$(hh_sm[s] and t0[t]).. 	uIdxFund[s]*vIdxFund[t]  =E=  uA_Dom[s] * vA[t,s];

# ----------------------------------------------------------------------------------------------------
#  Define B_HH_vA0_Calib model
# ----------------------------------------------------------------------------------------------------
Model B_HH_vA0_Calib /
E_HH_vA0_Calib_vAt0
/;


# ----------------------------------------------B_HH_vA0----------------------------------------------
#  Initialize B_HH_vA0 equation block
# ----------------------------------------------------------------------------------------------------
EQUATION E_HH_vA0_vAt0[t,s];
E_HH_vA0_vAt0[t,s]$(hh_sm[s] and t0[t]).. 	vA[t,s]	 =E=  uIdxFund[s] * vIdxFund[t]+vA_F[s];

# ----------------------------------------------------------------------------------------------------
#  Define B_HH_vA0 model
# ----------------------------------------------------------------------------------------------------
Model B_HH_vA0 /
E_HH_vA0_vAt0
/;
...
# ----------------------------------------------------------------------------------------------------
EQUATION E_HH_pWedge_labTax[t,s,n];
E_HH_pWedge_labTax[t,s,n]$(hh_l[s,n] and txe[t]).. 		pS[t,s,n]		 =E=  p[t,n]*(1-tauS[t,s,n]);
EQUATION E_HH_pWedge_inpTax[t,s,n];
E_HH_pWedge_inpTax[t,s,n]$(hh_input[s,n] and txe[t]).. 	pD[t,s,n]		 =E=  p[t,n]*(1+tauD[t,s,n]);
...
E_HH_pWedge_sp[t,s]$(hh_sm[s] and txe[t]).. 				sp[t,s]			 =E=  sum(n$(HH_L[s,n]), p[t,n]*qS[t,s,n])-sum(n$(HH_input[s,n]), p[t,n]*qD[t,s,n])-TotalTax[t,s];

# ----------------------------------------------------------------------------------------------------
#  Define B_HH_pWedge model
# ----------------------------------------------------------------------------------------------------
Model B_HH_pWedge /
E_HH_pWedge_labTax, E_HH_pWedge_inpTax, E_HH_pWedge_taxRev, E_HH_pWedge_sp
/;
...
mu.fx[s,n,nn]$(P_map[s,n,nn]) = mu.l[s,n,nn]$(P_map[s,n,nn]);
...
vA.fx[t,s]$((HH_sm[s] and t0[t])) = vA.l[t,s]$((HH_sm[s] and t0[t]));
...
vA.lo[t,s]$((HH_sm[s] and tx0[t])) = -inf;
vA.up[t,s]$((HH_sm[s] and tx0[t])) = inf;
...
vA_F.lo[s]$(HH_sm[s]) = -inf;
vA_F.up[s]$(HH_sm[s]) = inf;
uIdxFund.lo[s]$(HH_sm[s]) = -inf;
uIdxFund.up[s]$(HH_sm[s]) = inf;
...
# solve:
 solve M_vLarge_2019_CGE_SimpleEOP_C using CNS;
