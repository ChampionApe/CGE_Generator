...
E_P_WS_pW[t,s,n]$(p_sm[s] and dwsn[s,n] and txe[t]).. 						pW[t,s,n]	 =E=  sum(m$(dWSnm[s,n,m]), uWS[t,s,n,m] * (uWS_D[t,s,m] * (sum(nn$(n2m_D[nn,m]), pD[t,s,nn])+sum(nn$(dWTy[s,nn]), uWTy[s,nn] * pD[t,s,nn])) + (1-uWS_D[t,s,m])* (sum(nn$(n2m_F[nn,m]), pD[t,s,nn])+sum(nn$(dWTyF[s,nn]), uWTyF[s,nn]*pD[t,s,nn]))));
...
# ----------------------------------------------------------------------------------------------------
Model B_P_WS /
E_P_WS_pW, E_P_WS_qWS, E_P_WS_qDWTD, E_P_WS_qDWTyD, E_P_WS_qDWTF, E_P_WS_qDWTyF
/;
...
# solve:
 solve M_vGRS2019CGE_B using CNS;
