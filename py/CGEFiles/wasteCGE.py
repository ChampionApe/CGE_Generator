...
class WasteCGE(CGE_base):
	...
	def stdProduction(self, tree, cl = 'getDynamicNCES', extension = 'emission', abateCosts = False, taxInstr = 'tauLump', partial = True, initFromGms = 'initFirmValueBlock', adjCosts = True, **kwargs):
...
class WasteManagementCGE(CGE_base):
	def steadyStateDepr(self):
		s = adj.rc_pd(adj.rc_pd(self.db('qD'), self.db('inv_p')), alias = {'n':'nn'})
		return adjMultiIndex.applyMult(s, self.db('dur2inv')).droplevel('nn')/adj.rc_pd(self.db('qD'), self.db('dur_p'))-self.db('g_LR')
	...
	def stdWasteManagement(self, tree, 
							...
							initFromGms = 'initFirmValueBlock', 
							adjCosts = True, **kwargs):
		m = self.addFromNest(mWaste, cl, tree, exoP = exoP, exoQS = exoQS, abateCosts = abateCosts, taxInstr = taxInstr, partial = partial, initFromGms = initFromGms, adjCosts = adjCosts, **kwargs)
		self.compiler.locals['RCTech'] = m.RCTech
		return m
	...
	def stdTrade(self, name, cl = 'Armington_waste', initFromGms = 'initArmingtonWasteParams', **kwargs):
		return self.addTrade(cl, name, self.db, initFromGms=initFromGms, **kwargs)
