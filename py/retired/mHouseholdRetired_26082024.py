...
class StaticConsumer(Model):
	...
	def initData(self):
		...
		self.db.aom(pd.Series(1, index = self.get('sm')), name = 'crra', priority='second')
...
class Ramsey(Model):
	...
	def initData(self):
		...
		# self.db.aom(self.computeSP(), name = 'sp', priority='first')
		self.db.aom(pd.Series(1, index = cpi([self.db('txE'), self.get('sm')])), name = 'qC', priority = 'first')
		self.db.aom(pd.Series(1, index = cpi([self.db('txE'), self.get('sm')])), name = 'discUtil', priority = 'first')
		self.db.aom(pd.Series(0, index = self.get('sm')), name = 'h_tvc', priority = 'first')
		self.db.aom(pd.Series(1, index = self.get('sm')), name = 'crra', priority='first')
	...
	#### 2. GROUPINGS AND MODEL SPECIFICATIONS: 
	def models(self, **kwargs):
		return OrdSet([f"B_{name}" for name in self.m])+OrdSet([f"B_{self.name}_{k}" for k in ('CRRA_GHH','pWedge','euler')])
	...
	@property
	def textBlocks(self):
		return self.nestingBlocks | {'CRRA_GHH': self.CRRA_GHH, 'pWedge': self.priceWedgeBlocks, 'euler': self.eulerBlocks}
	@property
	def nestingBlocks(self):
		return {name: getattr(gamsHouseholds, m.f)(name) for name, m in self.m.items()}
	@property
	def eulerBlocks(self):
		return gamsHouseholds.simpleDynamic(f'{self.name}_euler', self.name)	
	@property
	def CRRA_GHH(self):
		return gamsHouseholds.CRRA_GHH(f'{self.name}_CRRA_GHH', self.name)
	@property
	def priceWedgeBlocks(self):
...
"""
	...
	@property
	def initText(self):
		return "" if self.initFromGms is None else getattr(gamsHouseholds, f'init_CRRA_GHH_{self.initFromGms}')(self.name)
...
""" 
	...
	def group_alwaysExo(self):
		if not self.partial:
			return Group(f'{self.name}_alwaysExo', v = [('sigma', self.g('kninp')),
													...
													('frisch', self.g('sm')),
													('crra', self.g('sm')),
													('Rrate', None), ('iRate', None), ('h_tvc', self.g('sm')),
													('vAssets', ('and', [self.g('sm'), self.g('t0')])),
													('tauD', self.g('input')),
													('tauS', self.g('L')),
													('tauLump', ('and', [self.g('sm'), self.g('tx0E')]))],
											sub_v = [('mu', self.g('endoMu'))])
	...
	def group_alwaysEndo(self):
		return Group(f'{self.name}_alwaysEndo', v = [('pD', ('or',  [self.g('int'), self.g('input')])),
													 ...
													 ('sp', ('and', [self.g('sm'), self.g('txE')])),
													 ('vAssets', ('and', [self.g('sm'), self.g('tx0')])),
	...
	def group_exoInCalib(self):
		return Group(f'{self.name}_exoInCalib', v = [('qD', ('and', [self.g('input'), self.g('t0')])),
													 ('qS', ('and', [self.g('L'), self.g('t0')])),
													 ('pD', ('and', [self.g('output'), self.g('t0')])),
													 ('vAssets', ('and', [self.g('sm'), self.g('t1')])),
													 ('TotalTax', ('and', [self.g('sm'), self.g('t0')]))])
	@property
	def group_endoInCalib(self):
		return Group(f'{self.name}_endoInCalib', v = [('mu', self.g('endoMu')),
