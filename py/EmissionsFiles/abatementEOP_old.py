...
class AbateCapGenTech(AbateSimple):
	def __init__(self, name, techType = "'lognormal'", initFromGms = None, ctype = 'SqrAdjCosts', **kwargs):
		""" AbateCapital but with technologies that may apply to more than one sector """
		""" ctype indicates type of adjustment costs. Currently only SqrAdjCosts are specified """
		super().__init__(name, techType = techType, **kwargs)
		self.initFromGms = initFromGms
		AbateCapGenTech.addProperties(self, ctype = ctype)
	...
	def addCtype(self, ctype):
		self.addProperty('ctype', ctype) # add specification of adjustment costs
		self.addProperty('addCosts', """-sum(tech$(dtech[s,tech]), divdEOP[t,s,tech])""") # NOTE: ADJUST THIS
