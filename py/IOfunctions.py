...
class readIO:
	def __init__(self, name = None, db = None, file_v = None, file_i = None, file_k = None, kwargs_v = None, kwargs_i=None, kwargs_k = None):
		...
		self.IO = {'v': pd.DataFrame(self.wb['v']['IO'].values),
				   'i': {k: pd.DataFrame(self.wb['i'][k].values) for k in DbFromExcel.sheetnames_from_wb(self.wb['i']) if not k.startswith(('told','customs'))},
		...
		self.addSettings_i(**noneInit(kwargs_i,{}))
		self.addSettings_k(**noneInit(kwargs_k,{}))
	...
	def addSettings_k(self, rMarkers=None, cMarkers=None, rowIndex = None, colIndex = None):
		self.s['k'] = { 'rMarkers': noneInit(rMarkers, {'init': {'ref': 'Investerende brancher', 'offset': {}}, 'end': {'ref': 'Total', 'offset': {}}}), 
						'cMarkers': noneInit(cMarkers, {'init': {'ref': 'Typer af durables', 'offset': {}}, 'end': {'ref': 'Total', 'offset':{}}}),
						'rowIndex': noneInit(self.IO['k'].iloc[:,0].astype(str), rowIndex),
						'colIndex': noneInit(self.IO['k'].iloc[2,:].astype(str), colIndex) }
	...
	@property
	def addIOi(self):
		self.db.aom(pd.DataFrame({k: extractRow(self.IO['i'][k], self.s['i']['rMarker'], self.s['i']['cMarkers'], r = self.s['i']['row'], c = self.s['i']['col'], rIndex = self.s['i']['rowIndex']) for k in self.IO['i']}).stack().rename_axis(index=['s','n']).reorder_levels(['n','s']), name = 'vD_inv')
	...
	def addIOk(self):
		...
		self.db.aom(self.blocks['k']['init/init'].stack().reorder_levels(['n','s']), name = 'vD_dur')
	...
	def addIOdepr(self):
		...
		self.db.aom(self.blocks['depr']['init/init'].stack().reorder_levels(['n','s']), name = 'vD_depr')
