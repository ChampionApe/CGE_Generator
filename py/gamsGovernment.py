...
def balancedBudget(name, m):
	return f"""
$BLOCK B_{name}
	E_{name}_pw[t,s,n]$({m}_input[s,n] and txE[t])..	pD[t,s,n]	=E= p[t,n]+tauD[t,s,n];
	E_{name}_taxRev[t,s]$({m}_sm[s] and txE[t])..	TotalTax[t,s]	=E= sum(n$({m}_input[s,n]), tauD[t,s,n] * qD[t,s,n]);
	E_{name}_bb[t,s]$({m}_sm[s] and txE[t])..			jTerm[s]	=E= sum(ss$(d_TotalTax[ss]), TotalTax[t,ss])-sum(n$({m}_input[s,n]), pD[t,s,n]*qD[t,s,n]);
$ENDBLOCK
"""
...
def taxCalibration(name, m):
	return f"""
$BLOCK B_{name}
	E_{name}_taxCal[t,s,n]$({m}_input[s,n] and txE[t])..	tauD[t,s,n]	=E= tauD0[t,s,n]*(1+taxRevPar[s]);
$ENDBLOCK
"""
