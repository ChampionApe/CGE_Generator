...
class balancedBudget(GmsPythonSimple):
	...
	def _symbols(self):
		...
		return ['pD','qD','mu','sigma','qnorm_inp','qiv_inp','tauS','tauD','tauD0','tauG_calib','TotalTax','p','jTerm','sp']
	...
	def adjust(self, standAlone=True, balanceInstrument='laborTax'):
		...
		self.m[f"{self.name}_dynamic"] = Submodule(**{'f': 'priceWedgeStatic'})
	...
	def _init_qD(self,m=None):
		return self.initSymbolFlat(.5, name = 'pD', indices = [self.get('txE'), self.get('input').union(self.get('int')).union(self.get('output'))])
	...
	def _init_qnorm_inp(self,m=None):
		return self.initSymbolFlat(0, name = 'qnorm_inp', indices = [self.get('txE'), self.get('kninp')])
	...
	def blocks(self, state):
		if state == 'B_standAlone':
			return OrdSet([f"B_{name}" for name in self.m])-OrdSet([f"B_{self.name}_bb",f"B_{self.name}_calib"])
		elif state == 'B':
			return OrdSet([f"B_{name}" for name in self.m])-OrdSet([f"B_{self.name}_calib"])
	...
	def g_endo(self, state):
		if state == 'B_standAlone':
			return OrdSet([f"G_{self.name}_endo_always", f"G_{self.name}_exo_in_calib",f"G_{self.name}_test"])
		elif state == 'B':
			return OrdSet([f"G_{self.name}_endo_always", f"G_{self.name}_exo_in_calib", f"G_{self.name}_balanceBudget",f"G_{self.name}_test"])
		elif state == 'C_standAlone':
			return OrdSet([f"G_{self.name}_endo_always", f"G_{self.name}_endo_in_calib"])
		elif state == 'C':
			return OrdSet([f"G_{self.name}_endo_always", f"G_{self.name}_endo_in_calib",f"G_{self.name}_balanceBudget_tx0",f"G_{self.name}_test"])
	...
	def g_exo(self, state):
		if state == 'B_standAlone':
			...
			return OrdSet([f"G_{self.name}_exo_always", f"G_{self.name}_exo_in_calib",f"G_{self.name}_test"])
		elif state == 'C':
			return OrdSet([f"G_{self.name}_exo_always", f"G_{self.name}_exo_in_calib",f"G_{self.name}_balanceBudget_t0"])
	...
	def _groups(self,m=None):
		return [gmsPy.Group(f"G_{self.name}_exo_always", 
			...
			v =[('qD', ('and', [self.g('input'), self.g('t0')])),
				...
				('tauS', ('and', [self.g('labor'), self.g('t0')]))]),
			...
			v =[('mu', self.g('endo_mu')),
				...
				gmsPy.Group(f"G_{self.name}_balanceBudget",
					g=[f"G_{self.name}_balanceBudget_tx0",f"G_{self.name}_balanceBudget_t0"]),
				gmsPy.Group(f"G_{self.name}_test",
					v=[('sp', ('and', [self.g('s'), self.g('t0')]))])
			]