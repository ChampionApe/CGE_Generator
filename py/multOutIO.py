...
class CreateIOTargets:
	""" Assume db contains IO with 1-to-1 correspondence: Sectors are uniquely associated with one output.
		Households demand materials, but does not supply them. """
	def __init__(self, dbIO, **kwargs):
		self.db = GpyDB(name = 'adjIO', ws = dbIO.ws, **kwargs)
		self.dbIO = dbIO

	def readSets(self):
		""" Split up aggregate 'F' sector into synthetic ones for each type of output from the sector. 
			Add subsets that define relevant domestic sectors/goods. """
		...
		self.db['nxF'] = self.db('vD0').index.levels[-1].difference(self.dbIO('n_F'))
		self.db['sxF'] = self.db('vD0').index.levels[0].difference(self.dbIO('s_f'))
	...
	def add_vD0syn(self):
		...
		self.db['vD0syn'] = pd.concat([adj.rc_pd(self.db('vD0'), ('not', self.dbIO('s_f'))), vD_F_mapped], axis = 0)
	...
	def add_vDnSyn_vSnSyn(self, vDnRaw, vSnRaw):
		...
		dS_shares = dS_netOfExp.div(tS_netOfExp, fill_value=0)
		fS_shares = 1-dS_shares
		vDn_DD = (adj.rc_pd(vDnRaw, ('not', self.dbIO('s_f'))) * (dS_shares[dS_shares>0])).dropna() # domestic demand for domestic goods
		vDn_DF = (adj.rc_pd(vDnRaw, ('not', self.dbIO('s_f'))) * (fS_shares[fS_shares>0])).dropna() # domestic demand for foreign goods
		demandShares = adj.rc_pd(vDn_DD, self.dbIO('s_p'))/adj.rc_pd(vDn_DD, self.dbIO('s_p')).groupby('n').sum()
		...
		μSn_D.index = μSn_D.index.set_levels(μSn_D.index.levels[0]+'_F', level = 's') # rename as foreign synthetic sectors
		...
		μSn_D_and_nOnlyF = μSn_D.combine_first((v_onlyFromF/v_onlyFromF.groupby('n').sum())) # combine shares from domestic distributions + uniform distribution for goods without domestic supply		
		...
		vSn_F.index = vSn_F.index.set_levels(vSn_F.index.levels[1]+'_F',level='n')
	...
	def add_vD(self):
		self.db['vD'] = adjMultiIndex.applyMult(adj.rc_pd(self.db('vDsyn'), self.db('synF2F')), self.db('synF2F')).groupby(['ss','n']).sum().rename_axis(['s','n']).combine_first(adj.rc_pd(self.db('vDsyn'), self.db('sxF')))

	def add_vDn(self):
		self.db['vDn'] = adjMultiIndex.applyMult(adj.rc_pd(self.db('vDnSyn'), self.db('synF2F')), self.db('synF2F')).groupby(['ss','n']).sum().rename_axis(['s','n']).combine_first(adj.rc_pd(self.db('vDnSyn'), self.db('sxF')))
	...
	def add_vS(self):
		...
		vS = adjMultiIndex.applyMult(adj.rc_pd(vS, self.db('synF2F')), self.db('synF2F')).groupby(['ss','n']).sum().rename_axis(['s','n']).combine_first(adj.rc_pd(vS, self.db('sxF')))
		self.db['vS'] = vS

	def add_vSn(self):
		self.db['vSn'] = adjMultiIndex.applyMult(adj.rc_pd(self.db('vSnSyn'), self.db('synF2F')), self.db('synF2F')).groupby(['ss','n']).sum().rename_axis(['s','n']).combine_first(adj.rc_pd(self.db('vSnSyn'), self.db('sxF')))
...
$BLOCK B_adjustIO
	...
	E_obj..					obj =E= sum([s,n]$(dvD0[s,n]), sqr((vD0[s,n]-vDsolve[s,n])/vD0[s,n]))+sum([s,n]$(dvS0[s,n]), sqr((vS0[s,n]-vSsolve[s,n])/vS0[s,n]));
$ENDBLOCK
