...
class Production(GmsPythonSimple):
	...
	def _symbols(self):
		...
		return ['pS','qS','pD','qD','mu','eta','sigma','qnorm_inp','qnorm_out','qiv_out','qiv_inp','Rrate','rDepr','icpar','K_tvc','ic','p','markup','tauS','tauD','outShare','TotalTax','tauLump','tauNonEnv','tauNonEnv0','taxRevPar','tauCO2','qCO2']
	...
	def addPriceWedge(self,f=None):
		...
		self.m[f"{self.name}_pWedge"] = Submodule(**{'f': pyDatabases.noneInit(f,'priceWedgeEmissions')})
		self.m[f"{self.name}_taxRev"] = Submodule(**{'f': 'taxCalibration'})
	...
	def _init_taxRevPar(self, m= None):
		return self.initSymbolFlat(0, name = 'taxRevPar', indices = self.get('s'))
	def _init_tauNonEnv(self, m= None):
		return self.initSymbolFlat(0, name = 'tauNonEnv', indices = [self.get('txE'), self.get('output',m=m)])
	def _init_tauNonEnv0(self, m= None):
		return self.initSymbolFlat(0, name = 'tauNonEnv0', indices = [self.get('txE'), self.get('output',m=m)])
	def _init_tauCO2(self, m=None):
		return self.initSymbolFlat(0, name = 'tauCO2', indices = [self.get('txE'), self.get('output',m=m)])
	def _init_qCO2(self, m= None):
		return self.initSymbolFlat(0, name = 'qCO2', indices = [self.get('txE'), self.get('output',m=m)])
	...
	def blocks(self, state):
		if state == 'C':
			...
			return OrdSet([f"B_{name}" for name in self.m])-OrdSet([f"B_{self.name}_taxRev"])
