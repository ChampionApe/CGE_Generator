...
class StaticNCES(GModel):
	...
	def readTree(self,tree):
		...
		self.db[self.n('output')] = self.get('output').remove_unused_levels()
		self.db[self.n('input')] = self.get('input').remove_unused_levels()
	...
	def initProperties(self, **kwargs):
		[self.addProperty(k,v) for k,v in ({'addMarginal': '', 'addCosts': '', 'addTax': '' ,'addMargCostInp': ''} | kwargs).items()]
