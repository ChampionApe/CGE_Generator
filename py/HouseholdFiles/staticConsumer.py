...
class StaticNCES(GModel):
	...
	def initProperties(self, **kwargs):
		[self.addProperty(k,v) for k,v in ({'incInstr': 'jTerm', 'taxInstr': 'tauLump', 'addInc_tx0': '', 'addInc_t0': '','addMargCostInp':''} | kwargs).items()]
	...
	def priceBlocks(self):
		return gamsHouseholds.priceBlock(f'{self.name}_price', self.name, addInc_t0 = self.addInc_t0, addInc_tx0 = self.addInc_tx0, addMargCostInp = self.addMargCostInp)
	@property
	def CRRA_vU(self):
...
class StaticGHH_waste(StaticGHH):
	def __init__(self, tree, wasteCosts = False, **kwargs):
		...
		StaticGHH_waste.addProperties(self, wasteCosts = wasteCosts)
