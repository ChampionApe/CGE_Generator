...
class AbateSimple(EmissionAccounts):
	...
	def initData(self):
		...
		self.db.aom(pd.Series(.1, index = self.db('techPot').index), name='techSmooth', priority='first') # this for lognormal
...
class AbateCapital(AbateSimple):
	...
	def initData(self):
		...
		self.db.aom(pd.Series(0, index = self.get('dtech')), name = 'KtvcEOP', priority='first')
