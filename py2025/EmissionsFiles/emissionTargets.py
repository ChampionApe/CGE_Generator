...
class EmissionTargets(GModel):
	def __init__(self, name, regulation = 'SYT_EXO', textTargetVariable = None, **kwargs):
		""" Three types of 'single-year targets' are implemented (regulation ∈ {'SYT_EXO','SYT_HR','SYT_OPT'}) 
		1. Taxes exogenous in non-binding years, endogenous taxes in binding years. (EXO)
		2. With hotelling rule between binding years. (HR).
		3. With numerically optimal path of taxes (requires welfare target be provided, OPT) """
		super().__init__(name = name, **kwargs)
		self.regulation = regulation
		self.textTargetVariable = noneInit(textTargetVariable, "0")

	def initData(self):
		self.db.aom(0, name='obj')
		self.db.aom(pd.Series(0, index = self.db('t')), name = 'tauCO2agg0', priority = 'first')
	# Initialize
	def initStuff(self, db=None, gdx=True):
		if db is not None:
			MergeDbs.merge(self.db, db)
	...
	@property
	def textBlocks(self):
		return {'emissionRegulation': ''.join([getattr(gamsEmissions, k)(self.name, targetVariable = self.textTargetVariable) for k in ('SYT','LRP','EB')])}
	...
	@property
	def group_alwaysExo(self):
		return Group(f'{self.name}_alwaysExo', v= [(f'qCO2_{k}', self.g(f't_{k}')) for k in ('SYT','LRP','EB')]+['tauCO2agg0'])
	@property
	def group_alwaysEndo(self):
		return Group(f'{self.name}_alwaysEndo', v= [('tauCO2agg', ('or', [self.g(f't_{k}') for k in ('SYT','LRP','EB')]+[self.g(f't_{k}_NB') for k in ('SYT','EB')]))]+['obj'])
	@property
	def group_exoInCalib(self):
		return Group(f'{self.name}_exoInCalib', v=[('tauCO2agg', self.g(f"t_{self.regulation.split('_')[0]}"))])
	@property
	def model_B(self):
		return OrdSet([f"B_{self.name}_{self.regulation}"])
	@property
	def model_C(self):
		return OrdSet([f"B_{self.name}_{self.regulation}_Calib"])

	# Adjust some of the standard writing methods: Add regulation statement. 
	def modelName(self, state = 'B'):
		return '_'.join(['M',self.name,state,self.regulation])
	@property
